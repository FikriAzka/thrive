...
.slider.round:before {
  border-radius: 50%;
                ...
                <form action="{{ route('ratings.store-data', $meeting) }}" method="POST">
                    ...
                    @auth
                        <div class="d-flex justify-content-end align-items-center mb-3">
                            ...
                            <label class="switch">
                                <input type="checkbox" id="toggleAccess" {{ Cache::get('allow_ratings', false) ? 'checked' : '' }}>
    ...
    document.getElementById('toggleAccess').addEventListener('change', function () {
        fetch("{{ route('ratings.toggle-access') }}", {
            method: "POST",
            headers: {
                "X-CSRF-TOKEN": "{{ csrf_token() }}",
                "Content-Type": "application/json"
            },
            body: JSON.stringify({ allow_ratings: this.checked })
